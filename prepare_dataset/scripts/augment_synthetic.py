...
def sanitize_bboxes(bboxes):
    """
    Исправляет мелкие ошибки в bounding boxes, возникающие из-за вычислений с плавающей точкой.
    Возвращает очищенные bboxes и список class_labels (без изменений).
    """
    if not bboxes:
        return [], []
    ...
    # 1. Клиппим координаты центров, чтобы они точно были в [0, 1]
    bboxes[:, 0] = np.clip(bboxes[:, 0], 0.0, 1.0) # x_center
    bboxes[:, 1] = np.clip(bboxes[:, 1], 0.0, 1.0) # y_center

    # 2. Фиксим ширину и высоту, чтобы bbox не вылезал за границы и не был слишком мелким
    # Это предотвращает предупреждения от Albumentations
...
def augment_synthetic_dataset(config: dict) -> Path:
    ...
    for img_path in tqdm(src_images, desc="  Аугментация"):
        ...
        if image is None:
            logger.warning(f"Не удалось прочитать изображение: {img_path.name}")
            failed += 1
            continue
        ...
        # ★★★ ВАЖНОЕ ИСПРАВЛЕНИЕ: очистка bbox'ов от ошибок плавающей точки ★★★

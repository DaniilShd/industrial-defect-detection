...
class ModelEvaluator:
    ...
    def _predict_image(self, img_path: Path, conf_threshold: float) -> Dict:
        ...
        if self.model_type == 'teacher':
            try:
                ...
                boxes = torch.tensor(result["bboxes"], device=self.device)
                scores = torch.tensor(result["scores"], device=self.device)
                labels = torch.tensor(result["labels"], device=self.device)
    ...
    def _compute_ap(
        ...
        for i, det in enumerate(detections):
            ...
            ious = box_iou(det_box.cpu(), gt_boxes.cpu())[0]
    ...
    def _compute_prf(
        ...
        for pred, gt in zip(predictions, ground_truths):
            ...
            ious = box_iou(pred["boxes"].cpu(), gt["boxes"].cpu())
            matched = set()

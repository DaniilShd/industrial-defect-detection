#!/usr/bin/env python3
"""Эксперимент 3: SSL-дообучение бэкбона + LT-DETR (Distillation)
SSL дообучение проводится методом distillation ИНДИВИДУАЛЬНО для каждого датасета
...
def _get_backbone_and_teacher(full_model: str) -> tuple:
    ...
    Пример:
      'dinov3/vits16-ltdetr-coco' → ('dinov3/vits16', 'dinov3/vitb16')
    """
...
def ssl_pretrain_for_dataset(
    ...
    Студент (ViT-S) учится у замороженного учителя (ViT-B) без разметки.
    """
    ...
    backbone, teacher = _get_backbone_and_teacher(full_model)  # student, teacher
    
    ssl_method = config['ssl'].get('method', 'distillation')
    ssl_epochs = config['ssl'].get('epochs_per_dataset', {}).get(
        ds_name, 
        config['ssl'].get('epochs', 400)
    )
    
    logger.info(f"SSL {ssl_method} for {ds_name}: {n_images} unlabeled images")
    logger.info(f"  Student: {backbone}")
    logger.info(f"  Teacher: {teacher}")
    logger.info(f"  Epochs: {ssl_epochs}, Batch: {config['ssl']['batch_size']}")
